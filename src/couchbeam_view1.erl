...
%% @copyright 2009 Benoît Chesneau.
%% @deprecated Please use the module {@link couch_view}.



-module(couchbeam_view1).
-author('Benoît Chesneau <benoitc@e-engura.org>').
...
init({Vname, Params, #db{server=ServerState, couchdb=CouchdbParams, base=BaseDB}=DbState}) -> 
    ...
    ViewState = #old_view{server    = ServerState, 
...
handle_call({parse_view, Refresh}, _From, State) ->
    {ViewResults, NewState} = is_fetch_view(Refresh, State),
    #old_view{fetched=Fetched} = NewState,
...
handle_call({count, Refresh}, _From, State) ->
    ...
    #old_view{fetched=Fetched, rows=Rows} = NewState,
...
handle_info({'EXIT', _Pid, _Reason}, State) ->
    io:format("Stopping view ~p ~n", [State#old_view.name]),
    {noreply, State};
...
is_fetch_view(Refresh, #old_view{couchdb=C, base=Base, params=Params, 
...
update_view_state(ViewResults, Refresh, #old_view{fetched=Fetched}=ViewState) ->
    if 
...
update_view_state1({ViewProps}=ViewResult, ViewState) ->  
    ...
    ViewState1 = ViewState#old_view{fetched  = true,
...
update_view_state1(ViewResult, ViewState) -> 
    ...
    ViewState2 = ViewState1#old_view{fetched = true,
                                 view_cache = ViewResult},
    ViewState2.
    
reset_view_state(ViewState) ->
    ViewState1 = ViewState#old_view{fetched  = false,
                                total_rows = 0,
                                offset = 0,
                                rows = [],
                                meta = []},
...
parse_view1(#old_view{view_cache={_ViewCache}, total_rows=TotalRows,offset=Offset, rows=Rows, meta=Meta}) ->
    Rows1 = [begin
        {Row1} = Row,
...
parse_view1(#old_view{view_cache=ViewCache, total_rows=_TotalRows,offset=_Offset, rows=_Rows, meta=_Meta}) ->
    ViewCache.
