...
-export([count1/1, fetch1/1, first1/1, fold1/2, foreach1/2]).
...
%% spec count1(View :: view()) -> integer()
%% @doc get number of results in the view
%% @deprecated use {@link couchbeam_view:count/3} instead.
count1(#view{db=Db,url=Url, options=Options, method=Method, body=Body,
        headers=Headers}) ->
...
%% @spec fetch1(View :: view()) -> term()
%% @deprecated use {@link couchbeam_view:fetch/3} instead.
fetch1(#view{db=Db, url=Url, method=Method, body=Body, headers=Headers}) ->
    #db{options=IbrowseOpts} = Db,
    case couchbeam:db_request(Method, Url, ["200"], IbrowseOpts, Headers, 
            Body) of
        {ok, _, _, RespBody} ->
