#!/usr/bin/env escript
%% -*- erlang -*-
%%! -pa ./ebin -pa ./t

main(_) ->
    etap:plan(15),
    start_app(),
    case (catch test()) of
        ok ->
            stop_test(),
            etap:end_tests();
        Other ->
            stop_test(),
            etap:diag(io_lib:format("Test died abnormally: ~p", [Other])),
            etap:bail()
    end,
    ok.

start_app() ->
    couchbeam:start(),
    Conn = couchbeam_server:start_connection_link(),
    catch couchbeam_server:delete_db(Conn, "couchbeam_testdb"),
    catch couchbeam_server:delete_db(Conn, "couchbeam_testdb2"),
    ok.
    
stop_test() ->
    Conn = couchbeam_server:start_connection_link(),

    catch couchbeam_server:delete_db(Conn, "couchbeam_testdb"),
    catch couchbeam_server:delete_db(Conn, "couchbeam_testdb2"),
    ok.
    
test() ->
    Conn = couchbeam_server:start_connection_link(),

    Db = couchbeam_server:create_db(Conn, "couchbeam_testdb"),
    etap:is(is_pid(Db), true, "db created ok"),
    DesignDoc = {[
        {<<"_id">>, <<"_design/couchbeam">>},
        {<<"language">>,<<"javascript">>},
        {<<"views">>,
            {[{<<"test">>,
                {[{<<"map">>,
                    <<"function (doc) {\n if (doc.type == \"test\") {\n emit(doc._id, doc);\n}\n}">>
                }]}
            },{<<"test2">>,
                {[{<<"map">>,
                    <<"function (doc) {\n if (doc.type == \"test2\") {\n emit(doc._id, null);\n}\n}">>
                }]}
            }]}
        }
    ]},
 
    Doc = {[
        {<<"type">>, <<"test">>}
    ]},
    couchbeam_db:save_doc(Db, DesignDoc),
    couchbeam_db:save_doc(Db, Doc),
    couchbeam_db:save_doc(Db, Doc),
    AllDocs = couchbeam_db:all_docs(Db, []),
    {T, _, _, R} = couchbeam_view1:parse_view(AllDocs),
    etap:is(length(R), 3, "all_docs: nb doc is ok"),
    etap:is(T, 3, "all_docs: TotalsRow ok"),
    AllDocs1 = couchbeam_db:all_docs(Db, [{"limit", "1"}]),
    {_, _, _, R1} = couchbeam_view1:parse_view(AllDocs1),
    etap:is(length(R1), 1, "nb doc is ok"),
    Doc1 = {[
        {<<"_id">>, <<"test">>},
        {<<"type">>, <<"test">>}
    ]},
    Doc2 = {[
        {<<"_id">>, <<"test2">>},
        {<<"type">>, <<"test">>}
    ]},
    Doc3 = {[
        {<<"_id">>, <<"test3">>},
        {<<"type">>, <<"test">>}
    ]},
    Doc4 = {[
         {<<"_id">>, <<"test4">>},
         {<<"type">>, <<"test2">>}
    ]},
    couchbeam_db:save_doc(Db, Doc1),
    couchbeam_db:save_doc(Db, Doc2),
    couchbeam_db:save_doc(Db, Doc3),
    couchbeam_db:save_doc(Db, Doc4),
    VResults = couchbeam_db:query_view(Db, {"couchbeam", "test"}, []),
    {T2, _, _, R2} = couchbeam_view1:parse_view(VResults),
    etap:is(T2, 5, "view: total_rows in view ok"),
    etap:is(length(R2), 5, "view: nb rows ok"),
    etap:is(couchbeam_view1:count(VResults), 5, "view count: nb rows ok"),
    VResults2 = couchbeam_db:query_view(Db, {"couchbeam", "test"}, [{"key", <<"test">>}]),
    etap:is(couchbeam_view1:count(VResults2), 1, "view, key : nb rows ok"),
    VResults3 = couchbeam_db:query_view(Db, {"couchbeam", "test"}, [{"startkey", <<"test">>}]),
    etap:is(couchbeam_view1:count(VResults3), 3, "view, startkey, nb rows ok"),
    VRresults4 = couchbeam_db:query_view(Db, {"couchbeam", "test"}, [{"startkey", <<"test">>}, {"endkey", <<"test2">>}]),
    etap:is(couchbeam_view1:count(VRresults4), 2, "view, startkey, endkey : nb rows ok"),
    VResults5 = couchbeam_db:query_view(Db, {"couchbeam",  "test"}, [{"keys", [<<"test">>, <<"test3">>]}]),
    etap:is(couchbeam_view1:count(VResults5), 2, "view, keys : nb rows ok"),
    {Id, _, _} = couchbeam_view1:first(VResults5),
    etap:is(Id, <<"test">>, "first key ok"),
    VResults6 = couchbeam_db:query_view(Db, {"couchbeam", "test2"}, [{"key", <<"test4">>},{"include_docs", true}]),
    etap:is(couchbeam_view1:count(VResults6), 1, "view, include_docs: nb rows ok"),
    
    {_, _, _, [R31|_]} = couchbeam_view1:parse_view(VResults6),
    etap:ok(case R31 of
        {_Id, _Key, _Value, _Doc} -> true;
        _ -> false
    end, "include_doc ok"),
    {_, _, _, {Doc41}} = R31,
    etap:is(proplists:get_value(<<"_id">>, Doc41), <<"test4">>, "include doc id ok"),
    ok.
